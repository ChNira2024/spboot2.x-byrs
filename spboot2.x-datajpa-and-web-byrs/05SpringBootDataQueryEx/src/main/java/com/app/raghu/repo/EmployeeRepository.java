...
public interface EmployeeRepository extends JpaRepository<Employee, Integer>
{
	...
	@Query("SELECT e.empName FROM Employee e WHERE e.empId=:id")  //named parameter style query
	Optional<String> getEmpNameById(Integer id);
	
	//SQL:5 fetch all columns and one row
	@Query("SELECT e FROM Employee e WHERE e.empId=:empId")  //named parameter style query
