...
public class TestOperationsRunner implements CommandLineRunner 
{
	...
	{
		 System.out.println(repo.getClass().getName());  //repository proxy class name
		...
		//---------------------------------------------------------
		//repo.deleteById(85);   //(or)
		//repo.deleteById(99);  //(or)
		
		/*
		  repo.delete(repo.findById(77).orElseThrow(()->new ProductNotFoundException(String.format(" -- %s NOT HAVING %d --", Product.class.getName(),77 )))			);
		*/
		//--------------------------------------
		repo.deleteAllById(Arrays.asList(10,11));  //this method call deleteById(id) in a loop for every id
		repo.deleteAll();	//this method fetch all the rows using findAll() method passing one by one object(for-loop) delete(obj) method
	}
}
